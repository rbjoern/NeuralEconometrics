...
parameters = {}
#Generel set-up
parameters['seed'] = 30032                #Seed for pseudo-random draws            
parameters['M'] = 100                 #Number of simulation repetitions
parameters['k'] = 1                     #Number of regressors
parameters['n'] = 10**3             #Number of observations 
...
parameters['filename'] = '5_3_v2_reg_prod_' +  str(datetime.date(datetime.now())).replace('-','_')
parameters['start_time']= datetime.now() #Time measure used for status emssages

#Specifications for how data is dra wn
...
### Define estimators
estimators = {}
estimators['MLE'] = {'name': 'MLE', 'estimator': est.estimator_mle_dgp_reg, 'est_kwargs':{},'bootstrapper': nn.bootstrap_estimator,
                      'mrg_kwargs':{}, 'fig_kwargs': {'color': 'xkcd:forest green', 'linestyle':':', 'linewidth': 2}}
estimators['OLS (I)'] = {'name': 'OLS (I)',     'estimator': est.estimator_ols, 'est_kwargs':{}, 'bootstrapper': nn.bootstrap_estimator,
                           'mrg_kwargs':{}, 'fig_kwargs': {'color': 'xkcd:light blue', 'linestyle':'--', 'linewidth': 2}}
estimators['OLS (II)'] = {'name': 'OLS (II)',     'estimator': est.estimator_ols_poly, 'est_kwargs':{}, 'bootstrapper': nn.bootstrap_estimator,
                           'mrg_kwargs':{}, 'fig_kwargs': {'color': 'xkcd:royal blue', 'linestyle':'--', 'linewidth': 2}}
...
estimators['NN (II)']    = {'name': 'NN (II)',        'estimator': nn.estimator_nn_reg, 'fig_kwargs': {'color': 'xkcd:dark purple', 'linestyle':'-', 'linewidth': 2},
                      'est_kwargs': {'layers': (100,100), 'activation': 'relu'}, 'bootstrapper': nn.bootstrap_estimator,
